...
BLEManagerLow::BLEManagerLow() : _server(nullptr), _isConnected(false) { _currentConfig = {LimitConfig(), false}; }
...
void BLEManagerLow::init(const std::string &deviceName) {
    ...
    _charCurrent = pService->createCharacteristic(CHAR_CURRENT_UUID, BLECharacteristic::PROPERTY_READ |
    ...
    _charCfgMaxTempPcb = pService->createCharacteristic(
            CHAR_CFG_MAX_TEMP_PCB_UUID, BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_WRITE);
...
void BLEManagerLow::updateTelemetry(float current, float pcbTemp, float ambTemp1, float ambTemp2,
