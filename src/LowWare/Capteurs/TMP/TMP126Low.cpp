...
TMP126Low::TMP126Low(uint8_t csPin, uint32_t spiFreq)
    : _csPin(csPin), _spiSettings(spiFreq, MSBFIRST, SPI_MODE0) {
    _spi = new SPIClass();
}
...
uint32_t TMP126Low::getMillis() const {
    return millis();
}

void TMP126Low::delayUs(uint32_t us) const {
